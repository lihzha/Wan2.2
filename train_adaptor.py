...
def build_sigma_schedule(pipe: WanTI2V, sampling_steps: int, shift: float):
...
def run_rollout_validation(
    pipe, adaptor, val_loader, sigmas, null_context, seq_len, mask2,
    train_guide_scale, fixed_eps=None,
):
    losses = []
    for batch in val_loader:
        bsz = batch["z_video"].shape[0]
...
def main():
    ...
    if args.fixed_z_init_path is not None:
        ...
        torch.save(fixed_eps.detach().cpu(), os.path.join(args.output_dir, "fixed_z_init.pt"))
        print(f"[train] FIXED-NOISE mode: one ε₀ (seed={args.seed}) reused every "
              f"step; saved output_dir/fixed_z_init.pt")
    ...
    with open(log_path, "w") as f:
        if args.loss_type == "rollout":
            f.write("step,loss,latent_mse,ctx_norm,z_pred_std,z_target_std,lr,grad_norm,wall_s\n")
    ...
    with ckpt_ctx:
        while step < args.total_steps:
            ...
            gnorm = torch.nn.utils.clip_grad_norm_(
                [p for p in adaptor.parameters() if p.grad is not None],
                args.grad_clip,
            )
            if args.loss_type == "rollout":
