#!/usr/bin/env bash
#SBATCH --job-name=act-droid
#SBATCH --output=slurm_outputs/action-droid/out_%x_%j.log
#SBATCH --error=slurm_outputs/action-droid/err_%x_%j.log
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
mkdir -p slurm_outputs/action-droid
...
RUN_TAG=${RUN_TAG:-${MODE}_${SAMPLING_STEPS}step_${NOISE_MODE}_${TOTAL_STEPS}steps_lr${LR}_seed${SEED}}
OUTPUT_DIR=${OUTPUT_DIR:-runs/action_droid_${RUN_TAG}}
...
echo "[launch] total_steps=${TOTAL_STEPS} batch_size=${BATCH_SIZE} seed=${SEED}"
echo "[launch] lr=${LR} warmup=${WARMUP_STEPS} lr_min_ratio=${LR_MIN_RATIO}"
echo "[launch] max_train_samples=${MAX_TRAIN_SAMPLES} max_val_samples=${MAX_VAL_SAMPLES}"
...
.venv/bin/python scripts/train_action_conditioned_wan_droid.py \
