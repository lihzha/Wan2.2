...
## 2026-06-12 00:20 PDT - Full-cache batch-5 step-5000 eval

Goal:
- Validate the fifth full-cache batch-size-5 checkpoint and continue the
  periodic eval cadence while the full 8-GPU DDP job remains queued.

Hypothesis:
- If training continues improving, validation and fixed held-out random-noise
  eval MSE should improve versus step `4000`, though the robot-motion blur may
  persist qualitatively.

Change:
- No source change. Copied `ckpt_latest.pt` to `ckpt_step5000.pt` after the
  step-5000 checkpoint timestamp advanced.
...
- eval job: `9579605`
- checkpoint:
  `runs/action_droid_side_bn512h8_L0-29_fresh_25step_window_top50_10k_lr5e-5_bs5/ckpt_step5000.pt`
- remote videos:
  `runs/action_droid_side_bn512h8_L0-29_fresh_25step_window_top50_10k_lr5e-5_bs5/videos_step5000_ep399_v0_s00004_s1000_1001/`
- 8-GPU full DDP job: `9565757`
...
- `ckpt_step5000.pt` timestamp:
  `2026-06-12 03:13:40 -0400`, size `958230026` bytes.
- `sacct -j 9579605` reports `COMPLETED 0:0`, elapsed `00:02:17`.
- Eval videos validate at 320x192, 33 frames, 16 FPS.
- step-5000 eval metrics:
  - seed1000 latent MSE `0.23440805077552795` vs null
    `1.9798624515533447`.
  - seed1001 latent MSE `0.23428887128829956` vs null
    `4.661829471588135`.
- local videos:
...
- Full 8-GPU job `9565757` remains pending on priority; latest estimate
  `2026-06-12T14:49:54` Della time on `della-i20g2`.

Analysis:
- Validation improved only slightly from step `4000` to `5000`
  (`0.2193350 -> 0.2164474`), but the fixed eval MSE improved more clearly
  (`0.2445 -> 0.2344` on seed1000 and `0.2477 -> 0.2343` on seed1001).
- Visual inspection shows the same qualitative failure mode: samples are far
  better than the null baselines and retain the table layout, but the moving
  robot/gripper still turns into a gray smeared cloud by frame 16/32.
...
## 2026-06-12 04:07 PDT - Full-cache batch-5 step-6000 eval

Goal:
- Validate the sixth full-cache batch-size-5 checkpoint and keep the periodic
  eval cadence active while the full 8-GPU DDP job remains queued.

Hypothesis:
- If training continues improving, validation should improve versus step
  `5000`; the fixed held-out eval may start flattening if the model has
  learned scene layout but not the high-frequency robot/gripper motion.

Change:
- No source change. Copied `ckpt_latest.pt` to `ckpt_step6000.pt` after the
  step-6000 checkpoint timestamp advanced.
...
- eval job: `9582155`
- checkpoint:
  `runs/action_droid_side_bn512h8_L0-29_fresh_25step_window_top50_10k_lr5e-5_bs5/ckpt_step6000.pt`
- remote videos:
...
- `sacct -j 9582155` reported clean completion; local logs were fetched from
  `slurm_outputs/action-droid/out_act-droid-win-eval-s6000_9582155.log` and
  `slurm_outputs/action-droid/err_act-droid-win-eval-s6000_9582155.log`.
- Eval videos validate at 320x192, 33 frames, 16 FPS.
- step-6000 eval metrics:
  - seed1000 latent MSE `0.23397839069366455` vs null
    `1.9798624515533447`.
  - seed1001 latent MSE `0.23458802700042725` vs null
    `4.661829471588135`.
- local videos:
...
- Full 8-GPU job `9565757` remains pending on priority; latest estimate
  `2026-06-12T16:51:21` Della time on `della-i24g2`.
- Single-GPU training job `9541718` remains running on `della-i21g3`; latest
  inspected log line is step `6000`.

Analysis:
- Validation still improved from step `5000` to `6000`
  (`0.2164474 -> 0.2090993`), so the training loss signal has not stalled.
- Fixed held-out eval MSE is essentially flat versus step `5000`
  (`0.2344 -> 0.2340` on seed1000 and `0.2343 -> 0.2346` on seed1001).
- Visual inspection matches the metric flattening: samples remain much better
  than null and keep the table/object layout, but the robot/gripper/motion
  region still becomes a gray smeared cloud by the middle/end frames. More
  single-GPU steps at this setting are not obviously fixing that failure mode.
...
## 2026-06-12 15:32 PDT - Full-cache batch-5 step-9000 eval

Goal:
- Validate the ninth full-cache batch-size-5 checkpoint and determine whether
  the fixed eval continues improving after the step-8000 validation bump.

Hypothesis:
- If step `8000` was a noisy validation bump rather than a real regression,
  step `9000` validation and fixed eval should recover or improve.

Change:
- No source change. Waited for step-9000 validation/checkpoint completion,
  copied `ckpt_latest.pt` to `ckpt_step9000.pt`, and submitted the fixed
  seed1000/seed1001 eval.
...
- `sacct -j 9601165` reports `COMPLETED 0:0`, elapsed `00:03:17`.
- Eval videos validate at 320x192, 33 frames, 16 FPS.
- step-9000 eval metrics:
  - seed1000 latent MSE `0.2113063931465149` vs null
    `1.9798624515533447`.
  - seed1001 latent MSE `0.2144833654165268` vs null
    `4.661829471588135`.
- local videos:
...
Analysis:
- Step `9000` is the best checkpoint numerically so far on both validation and
  the fixed held-out eval. Validation recovered from the step-8000 bump
  (`0.2159892 -> 0.2022871`), and fixed eval improved substantially versus
  step `8000`: seed1000 `0.2243548 -> 0.2113064`, seed1001
  `0.2300095 -> 0.2144834`.
- Visual quality is stable-to-slightly-improved, but not a dramatic perceptual
  jump from step `8000`; samples stay far better than null, while the
  seed1001 gripper/object interaction region still has gray haze/softening.
- The single-GPU job is unlikely to reach step `10000` before its
  `2026-06-12T19:58:30` Della wall-time limit. It may continue producing
  useful train logs, but there is no planned eval checkpoint before 10k.

Next:
- Keep monitoring single-GPU training job `9541718` until it reaches wall time,
...
- Latest checked `squeue --start` estimate:
  `2026-06-14T03:36:00` Della time on `della-i23g3`.
...
Goal:
- Refresh the active DDP queue state after the projection stabilized at Monday
  afternoon Della time.
...
- latest checked start estimate:
  `2026-06-15T13:12:00` Della time on `della-i19g1`
- full-run stdout/stderr files still do not exist.

Analysis:
- The 8-GPU job remains correctly queued but externally scheduler-delayed.
  There is still no training-code signal to inspect.
...
Goal:
- Refresh the active DDP queue state after the scheduler projection slipped by
  another day.
...
- `scontrol show job 9565757`:
  ...
  - `Priority=3194`
  - `ReqTRES=cpu=64,mem=720G,node=1,billing=96,gres/gpu=8`
  - `SchedNodeList=della-i19g1`
- Latest checked start estimate:
  `2026-06-15T05:18:00` Della time on `della-i19g1`.
- Full-run stdout/stderr files still do not exist.

Analysis:
- This remains external scheduler delay. The job has not started, failed, or
  produced logs. There is no code-level issue to debug yet.
